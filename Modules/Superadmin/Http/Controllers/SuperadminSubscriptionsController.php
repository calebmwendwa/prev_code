...
class SuperadminSubscriptionsController extends BaseController
{
    ...
    public function create()
    {
        ...
        // CM added package categories
        $package_categories = PackageCategory::forDropDown();

        return view('superadmin::superadmin_subscription.add_subscription')
              ->with(compact('packages', 'business_id', 'gateways', 'package_categories'));
    }
    ...
    public function editSubscription($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('superadmin::superadmin_subscription.edit_date_modal')
                        ->with(compact('subscription', 'package_categories'));
        }
    }
...
//    CM get package category
    public function getPackageByCategory(Request $request)
    {
        try {
            $package_category_id = $request->get('package_category_id');
            $packages = collect();

            if (!empty($package_category_id)) {
                $packages = Package::where('package_category_id', $package_category_id)
                    ->get(['name', 'id'])
                    ->pluck('name', 'id');
            }
            ...
            $output = [
                'success' => true,
                'package_html' => view('superadmin::superadmin_subscription.partials.category_dropdown', compact('packages'))->render()
            ];
