...
class BaseController extends Controller
{
    ...
    public function _add_subscription($code, $price, $business_id, $package, $gateway, $payment_transaction_id, $user_id, $is_superadmin = false)
    {
        ...
        // SK13092022 Moved PesaPal to Auto Authorize
        //   if ($package->price != 0 && (in_array($gateway, ['offline', 'pesapal']) && !$is_superadmin)) {
        if ($package->price != 0 && (in_array($gateway, ['offline']) && !$is_superadmin)) {
    ...
    Public function _addtrial_subscription($business_id, $package, $gateway, $payment_transaction_id, $user_id, $is_superadmin = false){
        if (!is_object($package)) {
            $package = Package::active()->find($package);
        }
        ...
        if ($package->price != 0 && (in_array($gateway, ['offline']) && !$is_superadmin)) {
            //If offline then dates will be decided when approved by superadmin
            //SK10062023 Pick trial dates for offline as well
            // $subscription['start_date'] = null;
            // $subscription['end_date'] = null;
            // $subscription['trial_end_date'] = null;
            // $subscription['status'] = 'waiting';
            $dates = $this->_get_package_trial_dates($business_id, $package);
        ...
        //SK10062023 Trial Price should be null{FREE}
        //$subscription['package_price'] = $package->price;
        $subscription['package_price'] = 0.00;
        $subscription['package_details'] = [
            'location_count' => $package->location_count,
            'user_count' => $package->user_count,
            'product_count' => $package->product_count,
    ...
    public function _get_package_trial_dates($business_id, $package){
        $output = ['start' => '', 'end' => '', 'trial' => ''];
        ...
        $trial_end_date = clone $start_date; // Create a copy of the start date
        $trial_end_date->addDays($package->trial_days);
        $output['trial'] = $trial_end_date->toDateString();

        // Set end date same as trial end date
        $output['end'] = $trial_end_date->toDateString();
