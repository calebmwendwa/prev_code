...
//        display packages related to category of the business
        $categories = Package::listPackagesByCategory(true);


        return view('superadmin::subscription.index')
            ->with(compact('packages', 'active', 'nexts', 'waiting', 'permission_formatted', 'intervals', 'categories'));
    }
    ...
    /**  add new subscription with trial period
     * @return Response
     */

    public function addtrial($package_id, $form_register = null)
    {
        if (!auth()->user()->can('superadmin.access_package_subscriptions')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            //SK10062023 No Payment for Trial Package
            $gateway = null;
            $payment_transaction_id = 'Free Trial Package';

            //Add subscription details after payment is succesful
            $this->_addtrial_subscription($business_id, $package_id, $gateway, $payment_transaction_id, $user_id);
            DB::commit();
    ...
    protected function confirm_pesapal($transaction_id, $request)
    {
        ...
        if ($pesapal_session['ref'] == $merchant_reference) {
            ...
            $output = ['success' => 1, 'msg' => __('superadmin::lang.subscription_added')];
