    ...
    <div class="modal-content edit-subscription-modal">
     {!! Form::open(['url' => action([\Modules\Superadmin\Http\Controllers\SuperadminSubscriptionsController::class, 'updateSubscription']), 'method' => 'POST', 'id' => 'edit_subscription_form']) !!}
      ...
      <div class="modal-footer">
        <button type="submit" class="tw-dw-btn tw-dw-btn-primary tw-text-white btn btn-primary">@lang( "superadmin::lang.update")</button>
        <button type="button" class="tw-dw-btn tw-dw-btn-neutral tw-text-white btn btn-danger" data-dismiss="modal">@lang( "superadmin::lang.close")</button>
      </div>
      {!! Form::close() !!}
    </div>
</div>
